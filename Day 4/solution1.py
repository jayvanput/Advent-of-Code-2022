...
if __name__ == "__main__":
    ...
    count_of_fully_contained_pairs = 0
    for pair in pairs:
        if (pair[0] <= pair[2] and pair[1] >= pair[3]) or (
            pair[2] <= pair[0] and pair[3] >= pair[1]):
            count_of_fully_contained_pairs += 1
    print(count_of_fully_contained_pairs)
...
As for solving, it solves in O(n) with a very hairy logical condition. 
"""