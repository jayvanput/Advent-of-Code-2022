...
if __name__ == "__main__":
    ...
    def helper(node: ListNode):
        if node.children:
            if node.size < 100000:
                small_directories.append(node.size)
            for child in node.children:
                helper(child)
    helper(root)
    print(sum(small_directories))
"""
Notes: Hardest part was building the tree but it was good practice for tree building. Then it's a simple tree traversal to build the directory sizes.
Then we just traverse again to find the ones smaller than 100000.
"""